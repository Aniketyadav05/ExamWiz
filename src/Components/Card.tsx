...
const Card: React.FC<CardProps> = ({ id, subject, Year, Degree, University }) => {
  return (
    <div className="group relative bg-[#1e293b]/60 border border-[#334155] rounded-2xl p-4 sm:p-6 shadow-xl backdrop-blur-md transition-all duration-300 cursor-pointer ease-in-out hover:shadow-[0_0_30px_#7c3aed99] transform hover:-translate-y-1 sm:hover:-translate-y-2 hover:scale-[1.01] max-w-full sm:max-w-md w-full text-left">
      ...
      <Link
        to={`/papers/${id}`}
        className="inline-block w-full text-xs md:text-l sm:w-auto text-center bg-gradient-to-r from-[#7c3aed] to-[#6366f1] hover:from-[#6d28d9] hover:to-[#4f46e5] text-white px-2 py-2 md:px-4 rounded-full font-semibold shadow-md hover:shadow-indigo-400/30 transition-all duration-300"
      >
        ✨ View Details →
      </Link>
